...
contract EnterprisePermissionTracker{
    ...
    //converts e12 to 12
    function checkReplace(string memory tag) internal pure returns(string memory){
    ...
    function processNameToInt(string memory allNames) public view onlyAdmin returns(string memory){
     ...
     modifier validateConductor(string memory _analysisId,string memory _entName){
      require(analysisMapping[_analysisId].enterprise.toSlice().compare(_entName.toSlice()) == 0);
  ...
  validateConductor(_analysisId, _entName)
  public view returns(string memory) {
  ...
  public view returns(string memory) {
   ...
   strings.slice memory next;
    ...
    if(fullPermissionObj.toSlice().compare("mon".toSlice()) == 0){
        return "monetizable";
    }
